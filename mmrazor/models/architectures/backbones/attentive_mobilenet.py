...
from mmrazor.models.architectures.ops.gml_mobilenet_series import (GMLMBBlock,
                                                                   GMLSELayer)
from mmrazor.models.mutables import OneShotMutableChannel, OneShotMutableValue, OneShotMutableChannelUnit, MutableChannelContainer
...
def _mutate_conv_module(
    ...
    if isinstance(conv_module, ConvModule):
        ...
        if mutable_kernel_size is not None:
            conv_module.bn.register_mutable_attr('kernel_size',
                                                mutable_kernel_size)
...
class AttentiveMobileNet(BaseBackbone):
    ...
    def __init__(self,
        ...
        # first
        first_mutable_out_channels = OneShotMutableChannel(
            num_channels=self.in_channels,
            candidate_choices=first_out_channels)

        self.last_mutable = first_mutable_out_channels
        ...
        final_mutable_out_channels = OneShotMutableChannel(
            num_channels=out_channels,
            candidate_choices=last_out_channels)

        OneShotMutableChannelUnit._register_channel_container(
            self, MutableChannelContainer) # OneShotMutableChannelUnit 一些dynamicOP的关系 

        _mutate_conv_module(
            self.first_conv, mutable_out_channels=first_mutable_out_channels)
        _mutate_conv_module(
        ...
        _mutate_conv_module(
            ...
            mutable_in_channels=derived_expand_channels,
            mutable_out_channels=final_mutable_out_channels)

        self.last_mutable = final_mutable_out_channels

    def mutate(self, out_channels_list, num_blocks_list, kernel_size_list,
        ...
        for i in range(max(num_blocks_list)):
            ...
            _mutate_mb_layer(self.layers[i], self.last_mutable, mutable_out_channels,
        ...
        self.dynamic_seq.register_mutable_attr('depth', mutable_depth)
    ...
    def make_layer(self, out_channels_list, num_blocks_list, kernel_size_list,
        ...
        for i in range(max(num_blocks_list)):
            ...
            self.layers.append(mb_layer)
            self.in_channels = out_channels

        self.dynamic_seq = DynamicSequential(*self.layers)
