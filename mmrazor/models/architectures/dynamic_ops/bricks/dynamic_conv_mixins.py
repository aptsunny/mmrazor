...
class DynamicConvMixin(DynamicChannelMixin):
    ...
    def _register_mutable_groups(
            self: _ConvNd, mutable_groups: BaseMutable) -> None:
    ...
    def _get_dynamic_params_by_mutable_channels(
        ...
        if 'groups' in self.mutable_attrs:
            ...
            # print([m.choices for m in mutable_groups.source_mutables])
            mask_ = torch.zeros(origin_choices).bool()
            # start_ = (origin_choices - num_channels_) // 2
            # mask_[start_:-start_] = True
            mask_[0:num_channels_] = True
            out_mask = mask_.to(weight.device)
            # in_mask = mutable_groups.current_mid_mask.to(weight.device)
        # else:
        #     in_mask = torch.ones(weight.size(1)).bool().to(weight.device)
        ...
        if self.groups == 1:
            ...
            # weight = weight[in_mask] # bias = self.bias[in_mask]
        else:
            raise NotImplementedError(
        ...
        bias = self.bias[out_mask].contiguous() if self.bias is not None else None
...
class BigNasConvMixin(DynamicConvMixin):
    ...
    def get_dynamic_params(
        ...
        weight, padding = self._get_dynamic_params_by_mutable_kernel_size(
            self.weight, is_depthwise=is_depthwise) # torch.Size([24, 1, 3, 3]) (1, 1)
    ...
    def _get_dynamic_params_by_mutable_kernel_size(
            self: _ConvNd, weight: Tensor, is_depthwise: bool) -> Tuple[Tensor, Tuple[int]]:
        """Get sliced weight and bias according to ``mutable_in_channels`` and
        ``mutable_out_channels``."""
    ...
    def forward_mixin(self: _ConvNd, x: Tensor) -> Tensor:
        ...
        groups = self.groups
        if self.groups == self.in_channels == self.out_channels:
            groups = x.size(1) # depthwise 
        weight, bias, padding = self.get_dynamic_params()
        # import pdb;pdb.set_trace() #             
        # import torch
        # torch.save(weight, '/mnt/lustre/sunyue1/autolink/workspace-547/0802_mmrazor/.base/data/x_weight.pkl')
        # dir(self.conv_func)

        # torch.save(weight, '/mnt/lustre/sunyue1/autolink/workspace-547/0802_mmrazor/.base/data/x_weight.pkl')
        # torch.save(self.conv_func(x, weight, bias, self.stride, padding, self.dilation, groups), '/mnt/lustre/sunyue1/autolink/workspace-547/0802_mmrazor/.base/data/conv_func.pkl')
        return self.conv_func(x, weight, bias, self.stride, padding,
                              self.dilation, groups)
